...
handlers.getPlayerProfile = function (args, context) {
    ...
    var profile;
    try {
        profile = server.GetPlayerProfile({ PlayFabId: args.playFabId });
    }
    catch (error) {
        log.error(error);
        return "group";
...
handlers.addMembers = function (args, context) {
    ...
    try {
        entity.AddMembers({ Group: group, Members: members, RoleId: "admins" });
    }
    catch (error) {
        log.error(error);
        return false;
...
handlers.GetOthersInventory = function (args, ID) {


    var inventory = server.GetUserInventory({ PlayFabId: args.ID });

    for (var i = 0; i < inventory.Inventory.length; i++) {
        if (inventory.Inventory[i].ItemClass == "tokens") {
...
handlers.additem = function (args) {
    var result = server.GrantItemsToUser({ PlayFabID: currentPlayerId, CatalogVersion: "Equipamentos", ItemIds: "EspadadeFerro" });
...
handlers.grantTokens = function (args, context) {

    var itemObject = args.ITEMS;
    var itemKeys = [];
    ...
    var result = server.GrantItemsToUser({
        ...
        CatalogVersion: "Tokens"
    });


    return { messageValue: args.ITEMS.length };
